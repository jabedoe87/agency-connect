...
type ErrorCase = 'A' | 'B' | 'C' | 'D' | 'E' | 'F';

const ERROR_MESSAGES: Record<ErrorCase, string> = {
  A: "This account uses Google sign-in. Use the 'Continue with Google' button below.",
  B: 'Incorrect email or password. Please try again.',
  C: 'No account found for this email address.',
  D: 'Please confirm your email before logging in.',
  E: 'Too many login attempts. Please wait a moment and try again.',
  F: 'Could not reach the login service. Try again in a moment.',
};
...
export default function Login() {
  ...
  const [lastErrorCase, setLastErrorCase] = useState<ErrorCase | null>(null);
  ...
  const hasError = lastErrorCase !== null;
        ...
        <form onSubmit={handleSubmit} className="space-y-4 glass-card p-6">
          ...
          {hasError && (
            <p className="text-sm text-muted-foreground text-center">
              Signed up with Google? Use the <span className="text-primary font-medium">'Continue with Google'</span> button instead.
            </p>
          )}
          ...
          <p className="text-xs text-muted-foreground text-center">Used Google to sign up? Continue with Google.</p>
