...
function logRegisterAuthInfo(message: string, extra?: Record<string, unknown>) {
  console.info(`[auth-register] ${message}`, extra ?? {});
}

function logRegisterAuthError(message: string, error: unknown) {
      ...
      <div className="w-full max-w-md fade-in">
        ...
        <form onSubmit={handleSubmit} className="space-y-4 glass-card p-6">
          ...
          <Button
            ...
            onClick={async () => {
              setLoading(true);
              logRegisterAuthInfo('starting Google OAuth', {
              ...
              if (result.redirected) {
                logRegisterAuthInfo('Google OAuth redirected to provider');
                return;
              }
              if (result.error) {
                const message = getErrorMessage(result.error);
                logRegisterAuthError('Google OAuth failed before session setup', result.error);
                toast({ title: 'Google sign-up failed', description: message, variant: 'destructive' });
                setLoading(false);
                return;
              }
              logRegisterAuthInfo('Google OAuth session set; redirecting to onboarding');
