...
CREATE INDEX ai_threads_user_updated ON public.ai_threads(user_id, updated_at DESC);
...
CREATE POLICY "own messages" ON public.ai_messages FOR ALL
  USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
CREATE INDEX ai_messages_thread_created ON public.ai_messages(thread_id, created_at ASC);
